			...
			<form>
				<fieldset>
					...
					<?php	
						...
						$dbconn=pg_connect();
						$queryhelp="select * from currentclass natural join regclasses join appuser on regclasses.instructor = appuser.user;";
                                		$result1=pg_prepare($dbconn, "uniq_user", $queryhelp);
                                		$result1=pg_execute($dbconn, "uniq_user",array());
					while ($row = pg_fetch_array($result)) {
							
						echo "<option value= $row[class]> $row[class] $row[firstname] $row[lastname] $</option>";						
						}
					?>
